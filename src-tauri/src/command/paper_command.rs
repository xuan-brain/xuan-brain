...
pub async fn get_pdf_attachment_base64(
    app_dirs: State<'_, AppDirs>,
    paper_id: i64,
) -> Result<PdfAttachmentBase64> {
    info!("Getting PDF attachment base64 for paper {}", paper_id);
    ...
    let pdf_contents = std::fs::read(&pdf_path).map_err(|e| {
        AppError::file_system(
    ...
    let base64_content = base64::encode(&pdf_contents);

    Ok(PdfAttachmentBase64 {
        file_name,
        base64_content,
        paper_title: paper.title,
    })
