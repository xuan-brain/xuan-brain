  ...
  async function saveGrobidServer() {
    ...
    if (editingGrobidServer.value) {
      newServers = newServers.map((s) =>
        s.id === editingGrobidServer.value.id ? { ...grobidForm.value, id: s.id } : s
      );
    } else {
      newServers.push({
        ...grobidForm.value,
...
<template>
  <v-card>
    <v-card-title>
      ...
      <v-spacer />
      <v-btn color="primary" size="small" variant="outlined" @click="openAddGrobidServer">
    ...
    <v-card-text>
      <v-list v-if="grobidServers.length === 0" class="text-grey pa-4 text-center">
      ...
      <v-list v-else>
        <v-list-item v-for="server in grobidServers" :key="server.id">
          <v-list-item-title>
            {{ server.name }}
            <v-chip v-if="server.is_active" size="x-small" color="success" class="ml-2">
              {{ t('settings.active') }}
            </v-chip>
          </v-list-item-title>
  ...
  <v-dialog v-model="grobidDialog" max-width="500">
    <v-card>
      <v-card-title>
        {{ editingGrobidServer ? t('dialog.edit') : t('dialog.add') }} Server
      </v-card-title>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click="saveGrobidServer">{{ t('dialog.save') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
