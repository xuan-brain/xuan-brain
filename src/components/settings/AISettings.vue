<script setup lang="ts">
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.choices && data.choices.length > 0) {
          ...
          alert(`连接成功但返回格式异常: ${JSON.stringify(data).substring(0, 100)}`);
        }
      } else {
  ...
  async function testCurrentConnection() {
    if (!llmForm.value.base_url || !llmForm.value.api_key || !llmForm.value.model_name) {
  ...
  const canTestConnection = computed(() => {
    return !!(llmForm.value.base_url && llmForm.value.api_key && llmForm.value.model_name);
  });
...
<template>
  <v-card>
    <v-card-title>
      ...
      <v-spacer />
      <v-btn color="primary" size="small" variant="outlined" @click="openAddLlmProvider">
    ...
    <v-card-text>
      <v-list v-if="llmProviders.length === 0" class="text-grey pa-4 text-center">
      ...
      <v-list v-else>
        <v-list-item v-for="provider in llmProviders" :key="provider.id">
          <v-list-item-title>
            {{ provider.name }}
            <v-chip v-if="provider.is_default" size="x-small" color="success" class="ml-2">
              {{ t('settings.default') }}
            </v-chip>
          </v-list-item-title>
  ...
  <v-dialog v-model="llmDialog" max-width="500">
    <v-card>
      <v-card-title>
        {{ editingLlmProvider ? t('dialog.edit') : t('dialog.add') }} Provider
      </v-card-title>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click="saveLlmProvider">{{ t('dialog.save') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
