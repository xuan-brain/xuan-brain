...
- [ ] PDF 渲染器选择
  - 评估选项：`pdfjs-dist` + 自定义渲染 vs `vue-pdf-embed`
  - 推荐使用 `vue-pdf-embed`（Vue 社区维护，快速集成）
- [ ] 阅读器 UI 组件
  ```
  src/components/reader/
  ├── PDFViewer.vue       # 主阅读器组件
  ├── PDFToolbar.vue      # 工具栏（缩放、翻页）
  ├── PDFSidebar.vue      # 侧边栏（目录、缩略图）
  └── PDFFullscreen.vue   # 全屏模式
  ```
- [ ] 核心阅读功能
  - 单页/双页模式
  - 缩放（25% - 400%）
...
- 使用 `vue-pdf-embed` 或 `pdfjs-dist`（配合 Vue 组件封装）
- 状态管理：当前页码、缩放比例、滚动位置（Pinia + 组合式 API）
- 使用虚拟滚动与懒加载优化长文档渲染
- 使用 `IntersectionObserver` 实现页面懒加载
...
# 安装前端工具
yarn add -D @types/vue-pdf-embed   # 类型定义
```
