<script setup lang="ts">
  ...
  // Dialog title
  const dialogTitle = computed(() => (isEditMode.value ? t('dialog.editTag') : t('dialog.addTag')));
  ...
  function handleKeyPress(event: KeyboardEvent) {
    if (event.key === 'Enter' && !loading.value && name.value.trim() && name.value.length <= 30) {
      handleSubmit();
    }
  }
</script>
      ...
      <v-card-text>
        ...
        <div class="mt-4">
          ...
          <div class="color-selector">
            ...
            >
              <v-icon v-if="selectedColor === colorOption.value" color="white" size="small">
                mdi-check
              </v-icon>
            </div>
          </div>
