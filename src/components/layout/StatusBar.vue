...
<template>
  <v-footer height="36" class="status-bar">
    ...
    <div class="status-bar-right">
      ...
      <v-menu v-model="showLLMMenu" location="top" :close-on-content-click="false">
        ...
        <v-list density="compact">
          ...
          >
            ...
            <v-list-item-subtitle>{{ provider.model_name }}</v-list-item-subtitle>
            <template #append v-if="provider.is_default">
              <v-chip size="x-small" color="success">{{ t('settings.default') }}</v-chip>
            </template>
          </v-list-item>
      ...
      <v-menu v-model="showGrobidMenu" location="top" :close-on-content-click="false">
        <template #activator="{ props }">
          <v-btn
            v-bind="props"
            size="small"
      ...
      <!-- Language selector -->
      <v-menu v-model="showLanguageMenu" location="top" :close-on-content-click="false">
        <template #activator="{ props }">
          <v-btn v-bind="props" size="small" variant="text" class="status-bar-btn">
            <span class="status-bar-flag">{{ availableLocales[currentLocale]?.flag || '🌐' }}</span>
            <v-icon size="small" class="ml-1">mdi-chevron-up</v-icon>
          </v-btn>
        </template>
        <v-list density="compact">
      ...
      <!-- Color selector -->
      <v-menu v-model="showColorMenu" location="top" :close-on-content-click="false">
        <template #activator="{ props }">
          <v-btn v-bind="props" size="small" variant="text" class="status-bar-btn">
            <div class="color-dot" :style="{ backgroundColor: appStore.accentColor }"></div>
            <v-icon size="small" class="ml-1">mdi-chevron-up</v-icon>
          </v-btn>
        </template>
        <v-list density="compact">
      ...
      <!-- Theme selector -->
      <v-menu v-model="showThemeMenu" location="top" :close-on-content-click="false">
        <template #activator="{ props }">
          <v-btn v-bind="props" size="small" variant="text" class="status-bar-btn">
            <v-icon size="small">
