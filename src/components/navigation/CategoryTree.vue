<script setup lang="ts">
  ...
  async function handleDeleteCategory() {
    ...
    if (!confirm(`确定要删除分类"${selectedNode.value.name}"及其所有子分类吗？此操作不可恢复。`)) {
      return;
    }
...
<template>
  <div class="category-tree">
    ...
    <div v-if="!loading && treeData.length === 0 && !errorMsg" class="tree-empty">
      <span class="text-caption text-grey mb-2">暂无分类</span>
      <v-btn size="small" variant="outlined" block @click="handleAddRootCategory">
    ...
    >
      <v-list density="compact">
        <v-list-item @click="handleAddSubcategory">
          ...
          <v-list-item-title>{{ t('dialog.addSubcategory') }}</v-list-item-title>
        </v-list-item>
